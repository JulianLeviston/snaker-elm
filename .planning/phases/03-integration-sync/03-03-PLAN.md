...
5. Watch toast notification — verify it auto-dismisses after approximately 3 seconds (CSS animation)
  </verify>
  <done>
Main.elm handles tick events with atomic state replacement. Join/leave events show toast notifications that auto-dismiss after 3 seconds. Scoreboard displays all players sorted by score.
  </done>
</task>
...
7. **New player state test:** Verify browser B shows BOTH snakes immediately (within one tick, ~100ms)

**Synchronization test:**

8. In browser A: Press arrow keys to change direction
9. Observe in browser B: Snake A moves in new direction within 100ms
10. Watch both browsers for 30 seconds
11. Verify: Snake positions match exactly in both windows (no drift)

**Apple test:**

12. Navigate a snake to eat an apple
13. Verify: Apple disappears in BOTH browsers simultaneously
14. Verify: Snake grows in BOTH browsers

**Notification timing test:**

15. Open browser C at http://localhost:4000
16. Observe in browser A: Toast notification appears
17. **Time the toast:** Verify it auto-dismisses after approximately 3 seconds (CSS animation handles this)

**Disconnect test:**

18. Close browser B tab
19. Observe in browser A: Toast "Player left", scoreboard shows appropriate player count
20. Verify: Browser B's snake disappears from browser A's view

**Expected results:**
- All position updates synchronize within one tick (100ms)
- No visual drift over time
- Join/leave notifications work and auto-dismiss after ~3 seconds
- Scoreboard updates in real-time
- New player sees all existing snakes immediately
  </how-to-verify>
...
4. Toast notifications appear on player join/leave and auto-dismiss after ~3 seconds
...
- New player sees existing snakes within one tick (~100ms)
