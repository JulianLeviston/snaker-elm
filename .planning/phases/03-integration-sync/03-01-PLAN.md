...
<task type="auto">
  ...
  <action>
Update serialize_snake/1 in GameServer to add two NEW fields (note: `name` is already serialized at line 331):
...
Test by checking GameServer.get_full_state/0 output includes new fields.
  ...
  <verify>
Run in iex to test both the field presence AND the invincibility timing:

1. Basic field check:
   `Snaker.GameServer.get_full_state()` — confirm each snake has `is_invincible`, `name`, and `state` fields.

2. Invincibility state change verification:
   - Join as new player: `{:ok, id} = Snaker.GameServer.join("TestPlayer")`
   - Get state immediately: check snake's `is_invincible` is `true`
   - Wait 2.5 seconds: `Process.sleep(2500)`
   - Get state again: check snake's `is_invincible` is now `false`
  </verify>
  <done>
Snake JSON serialization includes is_invincible (boolean that transitions from true to false after spawn timeout), name (string, already present), and state ("alive"/"dying") fields.
  </done>
</task>
...
3. New snake has is_invincible=true, after 2+ seconds has is_invincible=false
4. `cd assets && npm run build` compiles CSS without errors
5. app.css contains @keyframes flash, @keyframes fadeOut, @keyframes slideInFadeOut
</verification>

<success_criteria>
- Snake serialization includes is_invincible (boolean), name (string), state (string)
- is_invincible correctly reflects spawn invincibility timing (true on spawn, false after timeout)
